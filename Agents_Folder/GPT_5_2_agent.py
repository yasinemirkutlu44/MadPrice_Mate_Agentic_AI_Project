...
class FrontierAgent(Agent):
    name = "OpenAI GPT 5.2 Frontier Agent"
    color = Agent.BLUE
    ...
    def __init__(self, collection):
        ...
        """
        self.log("Starting OpenAI GPT 5.2 Frontier Agent")
        self.client = OpenAI()
        self.MODEL = "gpt-5.2"
        self.log("OpenAI GPT 5.2 Frontier Agent is setting up with OpenAI")
        self.collection = collection
        self.model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2")
        self.log("OpenAI GPT 5.2 Frontier Agent is ready")
    ...
    def find_similars(self, description: str):
        ...
        """
        self.log(
            "OpenAI GPT 5.2 Frontier Agent is performing a RAG search of the Chroma datastore to find 10 similar products"
        )
        vector = self.model.encode([description])
        results = self.collection.query(query_embeddings=vector.astype(float).tolist(), n_results=10)
        documents = results["documents"][0][:]
        prices = [m["price"] for m in results["metadatas"][0][:]]
        self.log("OpenAI GPT 5.2 Frontier Agent has found similar products")
    ...
    def price(self, description: str) -> float:
        ...
        by looking up 10 similar products and including them in the prompt to give context
        :param description: a description of the product
        :return: an estimate of the price
        """
        documents, prices = self.find_similars(description)
        self.log(
            f"OpenAI GPT 5.2 Frontier Agent is about to call {self.MODEL} with context including 10 similar products"
        )
        ...
        self.log(f"OpenAI GPT 5.2 Frontier Agent completed - predicting ${result:.2f}")
        return result
