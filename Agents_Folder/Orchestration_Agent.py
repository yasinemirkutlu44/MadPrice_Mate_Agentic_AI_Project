...
from Agents_Folder.Deal_Scraper import Deal, Opportunity
from Agents_Folder.Scraping_Agent import ScannerAgent
from Agents_Folder.Ensemble_GPT5_2_Llama import EnsembleAgent
from Agents_Folder.Notification_Message_Agent import MessagingAgent
...
class AutonomousPlanningAgent(Agent):
    ...
    def __init__(self, collection): #Overall orchestraion process.
        """
        Create instances of the 3 Agents that this planner coordinates across
        """
        self.log("Autonomous Planning Agent is initializing")
        self.scanner = ScannerAgent() # FIRST STEP: SCAN FOR DEALS
        self.ensemble = EnsembleAgent(collection) # SECOND STEP: ESTIMATE TRUE VALUE
        self.messenger = MessagingAgent() # FINAL STEP: NOTIFY USER
        self.openai = OpenAI()
        self.memory = None
