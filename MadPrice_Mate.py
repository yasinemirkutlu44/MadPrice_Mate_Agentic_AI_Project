...
import time
import gradio as gr
from Agents_Folder.deal_agent_framework import DealAgentFramework
...
class App:
    ...
    def run(self):
        with gr.Blocks(title="MadPrice Mate", fill_width=True) as ui:
            log_data = gr.State([])
            ...
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:24px"><strong>MadPrice Mate</strong> 💸💰 — Autonomous Agent Framework that retrieves bargains👀🤝</div>'
                )
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:14px">A proprietary fine-tuned LLM along with GPT 5.2 deployed on Modal and a RAG pipeline with a frontier model collaborate to send push notifications with great online deals 🛍️</div>'
                )
            with gr.Row():
                opportunities_dataframe = gr.Dataframe(
                    headers=["Bargains 🛒", "Price 💵", "Predicted 🧾", "Discount 🤑", "Product URL 🔗"],
                    wrap=True,
